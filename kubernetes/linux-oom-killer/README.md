...
## Production Incident Example 1: JVM Heap Looked Safe

Symptom:

```text
payments-api restarted three times in 30 minutes.
Dashboards showed JVM heap max at 700 MiB with a 1 GiB container limit.
Team assumed the OOM was impossible.
```

Investigation:

```shell
kubectl describe pod -n payments payments-api-...
kubectl logs -n payments payments-api-... --previous
kubectl exec -n payments payments-api-... -- cat /sys/fs/cgroup/memory.stat
```
...
The JVM heap was not the whole process. Native memory, thread stacks, direct
buffers, metaspace, JIT code cache, and file cache pushed the cgroup over the
limit.

Fix:

- Set `-XX:MaxRAMPercentage` lower than the container limit.
- Cap direct memory with `-XX:MaxDirectMemorySize` where appropriate.
- Reduce thread count.
- Keep memory limit high enough for non-heap overhead.
- Alert on container memory, not only JVM heap.

## Production Incident Example 2: BestEffort Debug Pod Died First

Symptom:

```text
A one-off debugging pod vanished during a traffic spike.
The app team thought Kubernetes killed the wrong workload.
```

Investigation:

```shell
kubectl get pod debug-shell -o jsonpath='{.status.qosClass}{"\n"}'
...
The debug pod had no requests or limits. Under node pressure, it had
`oom_score_adj=1000` and was the least protected workload on the node.
...
## Production Incident Example 3: Page Cache From Batch Jobs

Symptom:

```text
Batch pods processing large parquet files OOMKilled even though process RSS
looked below the limit.
```

Investigation:

```shell
kubectl exec -n analytics parquet-loader-... -- cat /sys/fs/cgroup/memory.stat
```
...
anon 268435456
file 1610612736
file_dirty 402653184
file_writeback 134217728
```

The job streamed large files through local disk and mmap-heavy libraries. File
cache was charged to the cgroup and dirty pages could not be reclaimed quickly
enough.

Fix:

- Raise the memory limit to include expected file cache.
- Reduce parallel file readers.
- Avoid mmap for this workload where possible.
- Use streaming APIs with bounded buffers.
- Put batch jobs on isolated nodes so reclaim does not hurt latency-sensitive
  services.

## Production Incident Example 4: Node-Level Slab Growth

Symptom:

```text
Several unrelated pods restarted on the same node.
No single pod showed abnormal memory growth.
```
...
```text
SUnreclaim grew from 600 MiB to 5 GiB during a retry storm.
slabtop showed conntrack-related caches dominating.
```
...
- Fix client backoff.
- Tune conntrack for the real traffic profile.
- Add node memory reservations.
- Add alerts for slab growth and PSI memory pressure.
